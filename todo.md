...
- [x] 从国家统计局“国家数据”导出 2010-01 至 2026-06 工业增加值与 PPI 月度历史。
- [x] 生成 `data/processed/nbs_iav_monthly.csv`，记录同比、累计同比、观测状态、来源、下载日期和原始 SHA-256。
- [x] 生成 `data/processed/nbs_ppi_monthly.csv`，记录原始同比指数、转换后同比、来源、下载日期和原始 SHA-256。
- [x] 处理 1—2 月合并发布：官方结构性空值保持为空，不插值。
- [x] 在来源登记、网页快照清单和导入元数据中记录 URL、下载/导出日期、SHA-256 与缓存状态。
...
- [x] 手工导出国家统计局工业增加值、PPI，并核验 1—2 月合并发布口径
...
- [x] 合并现有公开宏观数据和事件阶段，生成七国建模面板并接入 NBS IAV/PPI
- [x] 输出数据质量报告并通过独立校验
...
- [x] 执行已触发的回退：Q1 选择 no-change；Q2 数据门禁通过但证据为 `INCONCLUSIVE`；Q3 中国代理值退出主排名
