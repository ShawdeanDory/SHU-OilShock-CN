...
OUTCOME_SPECS = {
    "china_iav_yoy_pct": "China industrial value-added yoy, pct",
    "china_ppi_yoy_pct": "China PPI yoy, pct",
    "china_cpi_yoy_pct": "China CPI yoy, pct",
    "china_fx_log_change_pct": "CNY per USD monthly log change, pct",
}
...
def ardl_baseline(monthly: pd.DataFrame, outcomes: list[str], warnings_log: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    for outcome in outcomes:
        frame = make_lags(base, "OilShock", 6)
        frame[f"{outcome}_lag1"] = frame[outcome].shift(1)
        regressors = [f"OilShock_lag{lag}" for lag in range(7)] + [f"{outcome}_lag1"] + control_cols
        ...
        lag_terms = [f"OilShock_lag{lag}" for lag in range(7)]
        ...
        for term_type, estimate, se in [
            ("short_run_lag0", float(fit.params["OilShock_lag0"]), float(fit.bse["OilShock_lag0"])),
            ("cumulative_lag0_6", cumulative, cumulative_se),
            ("long_run_multiplier", long_run, np.nan),
        ]:
            rows.append(
                {
                    "outcome": outcome,
                    ...
                    "pvalue": float(fit.pvalues["OilShock_lag0"]) if term_type == "short_run_lag0" else np.nan,
                    "model": "ARDL",
                    "horizon": 0,
                    "specification": "y_lag1 + OilShock lags 0..6 + USD + GPR + month + COVID",
...
def block_bootstrap_coefs(
    ...
    for _ in range(reps):
        ...
        try:
            fit = fit_ols_hac(sample[y_col], sample[x_cols].astype(float), maxlags=maxlags)
            values.append(float(fit.params.get(term, np.nan)))
        except Exception:
...
def local_projection(monthly: pd.DataFrame, outcomes: list[str], warnings_log: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    month_cols = [column for column in base.columns if column.startswith("month_") and column != "month_end"]
    control_cols = ["OilShock_lag1", "usd_broad_index_log_return", "GPR_z", "covid_phase"] + month_cols
    base["OilShock_lag1"] = base["OilShock"].shift(1)
    ...
    for outcome in outcomes:
        ...
        for horizon in range(13):
            ...
            regressors = ["OilShock", f"{outcome}_lag1"] + control_cols
            usable = frame.dropna(subset=[target_col] + regressors).copy()
            if len(usable) < len(regressors) + 12:
                warnings_log.append({"code": "q2_lp_skipped", "message": f"{outcome} h={horizon}: too few observations."})
                continue
            fit = fit_ols_hac(usable[target_col], usable[regressors].astype(float), maxlags=horizon + 1)
            estimate = float(fit.params["OilShock"])
            se = float(fit.bse["OilShock"])
            boot = block_bootstrap_coefs(usable, target_col, regressors, "OilShock", horizon + 1)
            boot = boot[np.isfinite(boot)]
            if len(boot) >= 20:
                lower_80, upper_80 = np.quantile(boot, [0.10, 0.90])
                lower_95, upper_95 = np.quantile(boot, [0.025, 0.975])
            else:
                lower_80, upper_80 = estimate - norm.ppf(0.90) * se, estimate + norm.ppf(0.90) * se
                lower_95, upper_95 = estimate - norm.ppf(0.975) * se, estimate + norm.ppf(0.975) * se
            rows.append(
                {
                    "outcome": outcome,
                    "horizon": horizon,
                    ...
                    "pvalue": float(fit.pvalues["OilShock"]),
                    "model": "LocalProjection",
                    "specification": "future outcome on OilShock with lagged outcome/shock, USD, GPR, month, COVID; block bootstrap intervals",
                    "sample_start": usable["period"].iloc[0],
                    "sample_end": usable["period"].iloc[-1],
                    "n": int(len(usable)),
                }
...
def asymmetry_check(monthly: pd.DataFrame, outcomes: list[str], warnings_log: list[dict[str, Any]]) -> pd.DataFrame:
    rows: list[dict[str, Any]] = []
    base = add_month_dummies(monthly)
    base["OilShock_pos"] = base["OilShock"].clip(lower=0)
    base["OilShock_neg"] = base["OilShock"].clip(upper=0)
    month_cols = [column for column in base.columns if column.startswith("month_") and column != "month_end"]
    for outcome in outcomes:
        frame = base.copy()
        frame[f"{outcome}_lag1"] = frame[outcome].shift(1)
        regressors = ["OilShock_pos", "OilShock_neg", f"{outcome}_lag1", "usd_broad_index_log_return", "GPR_z", "covid_phase"] + month_cols
        usable = frame.dropna(subset=[outcome] + regressors)
        if len(usable) < len(regressors) + 12:
            continue
        try:
            fit = fit_ols_hac(usable[outcome], usable[regressors].astype(float), maxlags=6)
            for term in ["OilShock_pos", "OilShock_neg"]:
                ...
                rows.append(
                    {
                        ...
                        "model": "NARDL_proxy",
                        "specification": "positive and negative OilShock split",
                        "sample_start": usable["period"].iloc[0],
                        "sample_end": usable["period"].iloc[-1],
                        "n": int(len(usable)),
                    }
            ...
            warnings_log.append({"code": "q2_asymmetry_failed", "message": f"{outcome}: {exc}"})
    result = pd.DataFrame(rows)
...
def lag_and_covid_robustness(monthly: pd.DataFrame, outcomes: list[str], warnings_log: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    for lag_max in [3, 6, 12]:
        for exclude_covid in [False, True]:
            ...
            for outcome in outcomes:
                frame = make_lags(base, "OilShock", lag_max)
                frame[f"{outcome}_lag1"] = frame[outcome].shift(1)
                lag_terms = [f"OilShock_lag{lag}" for lag in range(lag_max + 1)]
                ...
                try:
                    ...
                    rows.append(
                        {
                            ...
                            "specification": f"OilShock lags 0..{lag_max}; exclude_covid={exclude_covid}",
                            "sample_start": usable["period"].iloc[0],
                            "sample_end": usable["period"].iloc[-1],
                            "n": int(len(usable)),
                        }
                    ...
                    warnings_log.append({"code": "q2_robustness_failed", "message": f"{outcome}: {exc}"})
    result = pd.DataFrame(rows)
...
def gdp_validation(quarterly: pd.DataFrame, warnings_log: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    usable = frame.dropna(subset=["china_real_gdp_yoy_pct", "gdp_lag1", "OilShock_sum", "GPR_z_mean"]) if "GPR_z_mean" in frame.columns else pd.DataFrame()
    if "GPR_z_mean" not in frame.columns:
        ...
        usable = frame.dropna(subset=["china_real_gdp_yoy_pct", "gdp_lag1", "OilShock_sum", "GPR_z_mean"])
    if len(usable) < 16:
            ...
            [
                {
                    ...
                    "lower_95": np.nan,
                    "upper_95": np.nan,
                    "correlation": float(frame[["china_real_gdp_yoy_pct", "OilShock_sum"]].dropna().corr().iloc[0, 1]) if frame[["china_real_gdp_yoy_pct", "OilShock_sum"]].dropna().shape[0] > 3 else np.nan,
                    "n": int(frame[["china_real_gdp_yoy_pct", "OilShock_sum"]].dropna().shape[0]),
                    "sample_start": "",
                    "sample_end": "",
        ...
        return result
    regressors = ["OilShock_sum", "gdp_lag1", "GPR_z_mean"]
    fit = fit_ols_hac(usable["china_real_gdp_yoy_pct"], usable[regressors].astype(float), maxlags=2)
    estimate = float(fit.params["OilShock_sum"])
    se = float(fit.bse["OilShock_sum"])
    result = pd.DataFrame(
        [
            {
                "outcome": "china_real_gdp_yoy_pct",
                ...
                "lower_95": estimate - norm.ppf(0.975) * se,
                "upper_95": estimate + norm.ppf(0.975) * se,
                "pvalue": float(fit.pvalues["OilShock_sum"]),
                "correlation": float(usable[["china_real_gdp_yoy_pct", "OilShock_sum"]].corr().iloc[0, 1]),
                "n": int(len(usable)),
                "sample_start": usable["quarter"].iloc[0],
                "sample_end": usable["quarter"].iloc[-1],
                "specification": "GDP yoy on quarterly OilShock sum, lagged GDP yoy and GPR",
            }
        ]
    )
    save_csv(result, "q2_gdp_validation.csv")
...
def plot_irf(irf: pd.DataFrame) -> None:
    ...
    for ax, outcome in zip(axes, outcomes, strict=False):
        ...
        ax.plot(frame["horizon"], frame["response"], marker="o", color=PALETTE["blue"], label="response")
        ax.fill_between(
            frame["horizon"],
            frame["lower_95"],
            frame["upper_95"],
        ...
        style_axis(ax, ylabel="response")
    axes[-1].set_xlabel("Months after oil shock")
    axes[0].legend(loc="upper right", handlelength=2.4)
    finish_figure(
        fig,
        title="Q2 local-projection responses to oil shocks",
        subtitle="Monthly China outcomes; shaded band reports 95% bootstrap/HAC interval, h=0..12.",
        source="Source: Q1 OilShock interface, OECD CPI, FRED FX/USD/GPR; generated by code/problem2/run_q2.py.",
        ...
        [
            {
                "component": "ARDL",
                "rows": len(ardl),
                "status": "PASS" if len(ardl) else "WARN",
                "note": "available monthly outcomes only",
            },
            {
                "component": "LocalProjection",
                "rows": len(irf),
                "status": "PASS" if len(irf) else "WARN",
                "note": "horizons 0..12 where estimable",
            },
            {
                "component": "GDPValidation",
                "rows": len(gdp),
                "status": "PASS" if len(gdp) else "WARN",
            ...
            },
            {
                ...
                "note": "positive and negative shock split",
            },
            {
