...
def plot_irf(irf: pd.DataFrame) -> None:
    ...
    fig, axes = plt.subplots(len(outcomes), 1, figsize=(9, max(4, 3 * len(outcomes))), sharex=True)
    if len(outcomes) == 1:
        axes = [axes]
    for ax, outcome in zip(axes, outcomes, strict=False):
        frame = irf.loc[irf["outcome"].eq(outcome)].sort_values("horizon")
        ax.axhline(0, color="black", linewidth=0.8)
        ax.plot(frame["horizon"], frame["response"], marker="o", label=outcome)
        ax.fill_between(frame["horizon"], frame["lower_95"], frame["upper_95"], alpha=0.18)
        ax.set_title(outcome)
        ax.set_ylabel("response")
    axes[-1].set_xlabel("months after shock")
    plt.tight_layout()
    for suffix in ["png", "pdf"]:
        plt.savefig(FIGURES_DIR / f"q2_irf.{suffix}", dpi=180)
    plt.close()
