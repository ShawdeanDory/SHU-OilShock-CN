...
def block_bootstrap_coefs(
    ...
    for _ in range(reps):
        ...
        candidate_x = sample[x_cols].astype(float)
        if candidate_x[term].nunique(dropna=True) < 2:
            values.append(np.nan)
            continue
        active_cols = [
            column
            for column in x_cols
            if column == term or candidate_x[column].nunique(dropna=True) > 1
        ]
        x = sm.add_constant(candidate_x[active_cols], has_constant="add").to_numpy(dtype=float)
        y = sample[y_col].to_numpy(dtype=float)
        while np.linalg.matrix_rank(x) < x.shape[1]:
            current_deficiency = x.shape[1] - np.linalg.matrix_rank(x)
            removable = [column for column in active_cols if column != term]
            removable.sort(key=lambda column: (not column.startswith("month_"), column))
            dropped = False
            for column in removable:
                trial_cols = [candidate for candidate in active_cols if candidate != column]
                trial_x = sm.add_constant(candidate_x[trial_cols], has_constant="add").to_numpy(dtype=float)
                trial_deficiency = trial_x.shape[1] - np.linalg.matrix_rank(trial_x)
                if trial_deficiency < current_deficiency:
                    active_cols = trial_cols
                    x = trial_x
                    dropped = True
                    break
            if not dropped:
        ...
        for shock_col in shocks:
            ...
            if len(usable) < len(regressors) + 16:
                ...
                for horizon, target_col in enumerate(target_cols):
                    ...
                    if len(boot) < max(100, int(0.80 * BOOTSTRAP_REPS)):
                        continue
                    se = float(np.std(boot, ddof=1))
                    lower_80, upper_80 = np.quantile(boot, [0.10, 0.90])
                    lower_95, upper_95 = np.quantile(boot, [0.025, 0.975])
                    joint_lower, joint_upper = np.quantile(
                        boot,
                        [simultaneous_alpha / 2.0, 1.0 - simultaneous_alpha / 2.0],
                    )
                    p_boot = min(
                        1.0,
                        2.0
                        * min(
                            float((1 + np.sum(boot <= 0.0)) / (len(boot) + 1)),
                            float((1 + np.sum(boot >= 0.0)) / (len(boot) + 1)),
                        ),
                    )
                    rows.append(
                        {
                            ...
                            "joint_lower_95": float(joint_lower),
                            "joint_upper_95": float(joint_upper),
                            "pvalue": float(p_boot),
                ...
                if fallback_rows == 0:
                    warnings_log.append(
                        {
                            "code": "q2_lp_skipped",
                            "message": f"{outcome} {shock_col}: too few observations for joint or horizon-specific LP.",
                        }
                    )
                continue
...
def plot_irf(irf: pd.DataFrame) -> None:
    ...
    finish_figure(
        ...
        source="来源：问题一结构冲击接口、国家统计局 IAV/PPI、OECD CPI、FRED 汇率/美元/GPR；由 code/problem2/run_q2.py 生成。",
