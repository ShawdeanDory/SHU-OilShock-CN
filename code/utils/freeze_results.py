...
def build_data_overview_figures() -> None:
    ...
    if monthly_path.exists():
        ...
        ax1.plot(monthly["month_end"], monthly["brent_usd_bbl"], color="black", label="Brent")
        ax1.set_ylabel("Brent USD/bbl")
        ax2 = ax1.twinx()
        ax2.plot(monthly["month_end"], monthly["GPR_z"], color="#c44e52", alpha=0.7, label="GPR z")
        ax2.set_ylabel("GPR z-score")
        ax1.set_title("Data overview: Brent and geopolitical risk")
        lines, labels = ax1.get_legend_handles_labels()
        lines2, labels2 = ax2.get_legend_handles_labels()
        ax1.legend(lines + lines2, labels + labels2, loc="upper left")
        fig.tight_layout()
        for suffix in ["png", "pdf"]:
            fig.savefig(FIGURES_DIR / f"data_overview_oil_gpr.{suffix}", dpi=180)
        plt.close(fig)
    if country_path.exists():
        ...
        for country, group in panel.groupby("country"):
            plt.plot(group["month_end"], group["fuel_index"], label=country)
        plt.title("Data overview: fuel price indexes")
        plt.ylabel("2010-01 = 100")
        plt.legend()
        plt.tight_layout()
        for suffix in ["png", "pdf"]:
            plt.savefig(FIGURES_DIR / f"data_overview_fuel_panel.{suffix}", dpi=180)
        plt.close()
