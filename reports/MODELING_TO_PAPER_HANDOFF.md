...
- E1_CAR_0：CAR=0.0797，经验 p=0.0244。
- E1_CAR_0_1：CAR=0.1561，经验 p=0.0244。
- E1_CAR_0_2：CAR=0.1355，经验 p=0.0513。
...
- 人民币原油成本：峰值/谷值 8.832，h=0，0—12月响应曲线面积 4.939 百分点·月，证据状态 INCONCLUSIVE。
- CPI：峰值/谷值 0.247，h=7，0—12月响应曲线面积 2.155 百分点·月，证据状态 INCONCLUSIVE。
- 工业增加值：峰值/谷值 -0.427，h=12，稀疏期限不计算曲线面积，证据状态 INCONCLUSIVE。
- PPI：峰值/谷值 0.762，h=10，0—12月响应曲线面积 7.401 百分点·月，证据状态 INCONCLUSIVE。
...
- six_month_pass_through_proxy_sensitivity：中国值 0.373，对照量 0.227，判断 CONDITIONAL_PROXY。
- control_minus_china_cpi_relative_response：中国值 不以0代替，对照量 0.245，判断 INCONCLUSIVE。
- control_minus_china_ip_relative_response：中国值 不以0代替，对照量 2.433，判断 INCONCLUSIVE。

政策关闭宏观反事实：

- 2026-06 CPI：无临时调控相对实际路径差额 0.414 个百分点，95%区间 [0.199, 1.088]。
- 2026-06 IAV：无临时调控相对实际路径差额 -0.469 个百分点，95%区间 [-1.690, 0.672]。
- 2026-06 PPI：无临时调控相对实际路径差额 2.596 个百分点，95%区间 [1.453, 3.633]。
...
- FHS_GJR_GARCH，h=1：平均分位损失 1.565，80%/90%覆盖率 0.809/0.936。
- Gaussian_random_walk，h=1：平均分位损失 1.629，80%/90%覆盖率 0.936/0.957。
- FHS_GJR_GARCH，h=3：平均分位损失 2.367，80%/90%覆盖率 0.872/0.957。
- Gaussian_random_walk，h=3：平均分位损失 2.562，80%/90%覆盖率 0.957/0.979。
- FHS_GJR_GARCH，h=6：平均分位损失 3.519，80%/90%覆盖率 0.894/0.936。
- Gaussian_random_walk，h=6：平均分位损失 3.689，80%/90%覆盖率 0.936/0.957。
...
- 2026-06 CPI：政策缓冲收益 0.414 个百分点，95%区间 [0.199, 1.088]。
- 2026-06 工业增加值：政策缓冲收益 0.469 个百分点，95%区间 [-0.672, 1.690]。
- 2026-06 PPI：政策缓冲收益 2.596 个百分点，95%区间 [1.453, 3.633]。

SAPR-CVaR 自适应调价规则：

- SAPR-CVaR 膝点规则：普通/压力/极端传导率 = (0.30, 0.10, 0.10)；压力阈值 436.0 元/吨，极端阈值 702.5 元/吨。

隔离检验样本策略比较：

- SAPR_CVaR_knee：检验样本宏观损失均值 0.165，95%CVaR 1.198，平均缺口月负担 0.022，调价波动率 0.035。
- full_mechanism：检验样本宏观损失均值 0.315，95%CVaR 1.957，平均缺口月负担 0.000，调价波动率 0.050。
- temporary_2026_approx：检验样本宏观损失均值 0.294，95%CVaR 1.768，平均缺口月负担 0.003，调价波动率 0.043。
- uniform_75_smoothing：检验样本宏观损失均值 0.281，95%CVaR 1.792，平均缺口月负担 0.005，调价波动率 0.044。

2026实际冲击情景策略比较：

- SAPR_CVaR_knee：2026情景宏观损失均值 1.712，95%CVaR 1.712，平均缺口月负担 0.097。
- actual_2026_event_path：2026情景宏观损失均值 1.648，95%CVaR 1.648，平均缺口月负担 0.093。
- full_mechanism：2026情景宏观损失均值 2.413，95%CVaR 2.413，平均缺口月负担 0.000。
- temporary_2026_approx：2026情景宏观损失均值 2.235，95%CVaR 2.235，平均缺口月负担 0.029。
- uniform_75_smoothing：2026情景宏观损失均值 2.236，95%CVaR 2.236，平均缺口月负担 0.027。

敏感性检验：

- bootstrap_block_12：规则=(0.30,0.10,0.10)，检验期非支配概率 1.000。
- bootstrap_block_3：规则=(0.30,0.10,0.10)，检验期非支配概率 1.000。
- bootstrap_block_6：规则=(0.30,0.10,0.10)，检验期非支配概率 1.000。
- cpi_weight_plus20：规则=(0.30,0.10,0.10)，检验期非支配概率 1.000。
- iav_weight_plus20：规则=(0.30,0.10,0.10)，检验期非支配概率 1.000。
- threshold_70_90：规则=(0.30,0.15,0.10)，检验期非支配概率 1.000。
- threshold_80_975：规则=(0.30,0.10,0.10)，检验期非支配概率 1.000。
