...
三项核心任务与一项自拟拓展的建模、检验、数值冻结和论文移交材料已完成。当前 `overall_status=PASS`，`paper_finalize_allowed=true`，阻塞门禁数为 0。
...
- FHS_GJR_GARCH，h=3：平均分位损失 2.301，80%/90%覆盖率 0.867/1.000。
- Gaussian_random_walk，h=3：平均分位损失 2.483，80%/90%覆盖率 1.000/1.000。
- FHS_GJR_GARCH，h=6：平均分位损失 3.886，80%/90%覆盖率 0.867/0.933。
